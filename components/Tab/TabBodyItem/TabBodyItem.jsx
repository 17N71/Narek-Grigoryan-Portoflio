    ...
    return (
      <React.Fragment>
        {tabData?.map(({ id, title, subtitle, date, desc }) => (
          <div
            className={`${tab.bodyItem} ${twdClasses} ${
              tabIsChanged ? "" : tab.actived
            } `}
            key={id}
          >
    ...
    <React.Fragment>
      {tabData?.map(({ id, title, percent }) => (
        <div
          ...
          key={id}
        >
          <div className={tab.headingSkills}>
            <h3>{title}</h3>
          </div>
          <div className={tab.range}>
            <span className="absolute right-0 -top-8 bg-gray-400 dark:bg-[#292725] text-black dark:text-[#906d49] px-2 py-[2px]">
              {percent}%
            </span>
            <div
              className={`${tab.rangeInner} `}
              style={{ width: tabIsChanged ? 0 + "%" : percent + "%" }}
            ></div>
          </div>
        </div>
      ))}
